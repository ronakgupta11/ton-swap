...
export async function run(provider: NetworkProvider) {
    ...
    let deployer = await provider.sender();
    ...
    let lop = provider.open(await LimitOrderProtocol.fromInit(deployer.address!));
    ...
    let factory = provider.open(await EscrowFactory.fromInit(deployer.address!, escrowCode.init!.code));
...
//     deployContracts(provider).catch(console.error);
// }
